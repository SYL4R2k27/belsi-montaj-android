...
interface UserRepository {
    ...
    /**
     * Sticky-binding пользователь<->объект.
     * siteObjectId=null → открепиться.
     * FIX(2026-05-01): объект сохраняется на сервере, переживает minimize/kill app.
     */
    suspend fun setCurrentObject(siteObjectId: UUID?): Result<User>
}
...
class UserRepositoryImpl @Inject constructor(
    ...
    override suspend fun getProfile(): Result<User> {
        return try {
            ...
            if (response.isSuccessful && response.body() != null) {
                ...
                var mergedUser = if (serverUser.role != null) {
                    serverUser
                } else {
                    serverUser.copy(role = localUser?.role)
                }

                // FIX(2026-05-03): на feature/driver-integration сервер ещё НЕ отдаёт
                // capabilities. Подмешиваем их локально для тестовых номеров,
                // чтобы переключатель ролей появился в UI. Удалить когда подключим
                // серверный endpoint.
                val testCaps = com.belsi.work.data.models.TestCapabilities.forPhone(mergedUser.phone)
                if (testCaps != null && mergedUser.capabilities.isEmpty()) {
                    mergedUser = mergedUser.copy(capabilities = testCaps)
                    android.util.Log.i(
                        "UserRepository",
                        "Injected test capabilities for ${mergedUser.phone}: $testCaps"
                    )
                }
    ...
    override suspend fun setCurrentObject(siteObjectId: UUID?): Result<User> {
        return try {
            val response = userApi.setCurrentObject(
                SetCurrentObjectRequest(siteObjectId = siteObjectId?.toString())
            )
            if (response.isSuccessful && response.body() != null) {
                val user = response.body()!!
                // Обновляем локальный кеш — currentSiteObjectId теперь sticky
                prefsManager.saveUser(user)
                Result.success(user)
            } else {
                Result.failure(Exception(parseErrorMessage(response.code())))
            ...
            Result.failure(Exception("Ошибка сохранения объекта: ${e.message}", e))
        }
    }
