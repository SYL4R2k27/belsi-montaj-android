...
class PendingSyncWorker @AssistedInject constructor(
    ...
    private suspend fun executeAction(entity: PendingActionEntity): Boolean {
        ...
        return when (action) {
            ...
            // FIX(2026-05-22) P0-2: отправляем сохранённый reason (break:lunch/break:smoke/null).
            // FIX(2026-05-22) P0-1: + реальное время старта (ISO) → сервер не схлопнет в NOW().
            is PendingAction.StartPause -> ok(pauseApi.startPause(
                StartPauseRequest(reason = action.reason, startedAt = isoFromMillis(action.startedAt))
            ))
            is PendingAction.EndPause -> ok(pauseApi.endPause())
            is PendingAction.StartIdle -> ok(pauseApi.startIdle(
                StartIdleRequest(reason = action.reason, startedAt = isoFromMillis(action.startedAt))
            ))
    ...
    /** FIX(2026-05-22) P0-1: epoch ms → ISO-8601 UTC (pydantic парсит как datetime). */
