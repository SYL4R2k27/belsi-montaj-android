...
interface CuratorRepository {
    ...
    suspend fun giveKudos(photoId: String, comment: String?): Result<Unit>
    ...
    suspend fun getAllUsers(role: String? = null, allRoles: Boolean = false, limit: Int = 200): Result<List<AllUserDto>>
...
class CuratorRepositoryImpl @Inject constructor(
    ...
    override suspend fun giveKudos(photoId: String, comment: String?): Result<Unit> {
        return try {
            val response = curatorApi.giveKudos(photoId, com.belsi.work.data.remote.api.KudosRequest(comment))
            if (response.isSuccessful) {
                Result.success(Unit)
            } else {
                Result.failure(Exception(parseErrorMessage(response.code())))
            ...
            Result.failure(Exception("Ошибка отправки благодарности: ${e.message}", e))
        }
    }
    ...
    override suspend fun getAllUsers(role: String?, allRoles: Boolean, limit: Int): Result<List<AllUserDto>> {
        return safeApiCall("пользователей") {
            curatorApi.getAllUsers(role = role, allRoles = allRoles, limit = limit)
        }
    }
