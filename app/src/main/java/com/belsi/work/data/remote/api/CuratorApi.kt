...
interface CuratorApi {
    ...
    /** POST /curator/photos/{photoId}/kudos — 👏 куратор хвалит монтажника (2.1.0) */
    @POST("curator/photos/{photoId}/kudos")
    ...
    suspend fun getAllUsers(
        ...
        @Query("all_roles") allRoles: Boolean = false,
        @Query("limit") limit: Int = 200,
    ...
    // ───────────────────────────────────────────────────────────
    // Shift admin: re-open / edit time / edit object / audit log
    // ───────────────────────────────────────────────────────────

    /** GET /curator/users/{userId}/shifts — список последних смен пользователя для редактирования */
    @GET("curator/users/{userId}/shifts")
    suspend fun getUserShiftsAdmin(
        @Path("userId") userId: String,
        @retrofit2.http.Query("limit") limit: Int = 50,
    ): Response<UserShiftsAdminResponse>

    /** POST /curator/shifts/{id}/reopen — re-open закрытой смены */
    @POST("curator/shifts/{shiftId}/reopen")
    suspend fun reopenShift(
        @Path("shiftId") shiftId: String,
        @Body request: ReopenShiftRequest = ReopenShiftRequest(),
    ): Response<ShiftAdminMutationResponse>

    /** PATCH /curator/shifts/{id} — редактировать start/finish/object */
    @retrofit2.http.PATCH("curator/shifts/{shiftId}")
    suspend fun editShift(
        @Path("shiftId") shiftId: String,
        @Body request: EditShiftRequest,
    ): Response<ShiftAdminMutationResponse>

    /** GET /curator/shifts/{id}/audit — история изменений */
    @GET("curator/shifts/{shiftId}/audit")
    suspend fun getShiftAudit(
        @Path("shiftId") shiftId: String,
    ): Response<ShiftAuditResponse>
...
@kotlinx.serialization.Serializable
data class UserShiftsAdminResponse(
    @kotlinx.serialization.SerialName("user_id")
    val userId: String,
...
data class AdminShiftDto(
    val id: String,
    val status: String,
    @kotlinx.serialization.SerialName("start_at") val startAt: String? = null,
    @kotlinx.serialization.SerialName("finish_at") val finishAt: String? = null,
    @kotlinx.serialization.SerialName("total_seconds") val totalSeconds: Int = 0,
    @kotlinx.serialization.SerialName("pause_seconds") val pauseSeconds: Int = 0,
    @kotlinx.serialization.SerialName("idle_seconds") val idleSeconds: Int = 0,
    // FIX(2026-05-18): backend теперь раздельно агрегирует lunch + smoke breaks.
    @kotlinx.serialization.SerialName("lunch_seconds") val lunchSeconds: Int = 0,
    @kotlinx.serialization.SerialName("break_seconds") val breakSeconds: Int = 0,
    @kotlinx.serialization.SerialName("site_object_id") val siteObjectId: String? = null,
    @kotlinx.serialization.SerialName("object_name") val objectName: String? = null,
    @kotlinx.serialization.SerialName("audit_count") val auditCount: Int = 0,
)

@kotlinx.serialization.Serializable
data class ReopenShiftRequest(
    val reason: String? = null,
)

@kotlinx.serialization.Serializable
data class EditShiftRequest(
    @kotlinx.serialization.SerialName("start_at") val startAt: String? = null,
    @kotlinx.serialization.SerialName("finish_at") val finishAt: String? = null,
    @kotlinx.serialization.SerialName("site_object_id") val siteObjectId: String? = null,
    val reason: String? = null,
...
@kotlinx.serialization.Serializable
data class ShiftAdminMutationResponse(
    val status: String,
    @kotlinx.serialization.SerialName("shift_id") val shiftId: String,
    @kotlinx.serialization.SerialName("new_state") val newState: kotlinx.serialization.json.JsonObject? = null,
)

@kotlinx.serialization.Serializable
data class ShiftAuditResponse(
    @kotlinx.serialization.SerialName("shift_id") val shiftId: String,
    val entries: List<ShiftAuditEntry> = emptyList(),
)

@kotlinx.serialization.Serializable
data class ShiftAuditEntry(
    val id: String,
    val action: String,
    @kotlinx.serialization.SerialName("performed_by_name") val performedByName: String? = null,
    @kotlinx.serialization.SerialName("performed_by_role") val performedByRole: String? = null,
    @kotlinx.serialization.SerialName("old_values") val oldValues: kotlinx.serialization.json.JsonObject? = null,
