...
class PhotoUploadWorker @AssistedInject constructor(
    ...
    private suspend fun uploadPhoto(photo: PhotoEntity): Boolean {
        ...
        try {
            ...
            // v3 привязка к окну/этапу — backend пишет site_object_window_photos.
            val windowIdBody = photo.windowId?.toRequestBody("text/plain".toMediaTypeOrNull())
            val stageBody = photo.stage?.toRequestBody("text/plain".toMediaTypeOrNull())
            val isFinalBody = if (photo.isFinal) "true".toRequestBody("text/plain".toMediaTypeOrNull()) else null
