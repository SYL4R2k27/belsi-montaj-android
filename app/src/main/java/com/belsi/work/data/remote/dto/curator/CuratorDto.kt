...
data class CuratorInstallerDto(
    ...
    /** Надёжное имя: first+last → full_name → телефон (full_name гарантирован бэк-триггером). */
    val displayName: String
        get() {
            val n = listOfNotNull(firstName, lastName).filter { it.isNotBlank() }.joinToString(" ")
            return n.ifBlank { fullName.ifBlank { phone } }
        }
}
...
data class CuratorDashboardDto(
    ...
    // Кто СЕЙЧАС на простое (idle, не break) — для экрана куратора «Сейчас».
    @SerialName("idle_now") val idleNow: Int = 0,
    @SerialName("idle_workers") val idleWorkers: List<IdleWorkerDto> = emptyList()
)

@Serializable
data class IdleWorkerDto(
...
data class CuratorPhotoDto(
    ...
    // 2.1.0: объект фото — для фильтра «Объект ▾» в фото-ленте (graceful null, если бэк не отдаёт)
...
data class CuratorToolTransactionDto(
    ...
    // FIX(2026-05-12) build17 P1: human-readable поля (JOIN на backend).
    @SerialName("tool_name") val toolName: String? = null,
    @SerialName("tool_serial_number") val toolSerialNumber: String? = null,
    @SerialName("installer_id") val installerId: String = "",
    @SerialName("installer_name") val installerName: String? = null,
    @SerialName("installer_phone") val installerPhone: String? = null,
    @SerialName("issued_by") val issuedBy: String = "",
    @SerialName("issued_by_name") val issuedByName: String? = null,
    ...
    @SerialName("returned_to_name") val returnedToName: String? = null,
...
data class UserDetailDto(
    ...
    // FIX(2026-05-11) BELSI 2.0.0: id текущей паузы нужен для AI-верификации простоя.
    // Поле опционально — пока backend не возвращает, кнопка AI-проверки не показывается.
    @SerialName("current_pause_id") val currentPauseId: String? = null,
    ...
    @SerialName("current_shift_lunch_seconds") val currentShiftLunchSeconds: Int = 0,
    @SerialName("current_shift_break_seconds") val currentShiftBreakSeconds: Int = 0,
