...
data class ShiftReportEntry(
    ...
    val lunchSeconds: Long = 0, // обед (break:lunch)
    val breakSeconds: Long = 0, // перекур (break:smoke)
    ...
    // FIX(2026-06-02) Report v2: бэк отдаёт эти поля для нового PDF/XLSX.
    // Default'ы для обратной совместимости — старый бэк не упадёт.
    val photosCount: Int = 0,        // кол-во фото за смену
    val siteObjectId: String? = null, // UUID объекта (для подсчёта уникальных)
    val userRole: String = ""         // для фильтра «только монтажники»
) {
    /**
