...
interface AuthApi {
    ...
    /**
     * FIX(2026-05-04): самостоятельная регистрация (V1 без email/SMS подтверждения).
     * POST /auth/signup
     * Бэкенд: auth_login.py
     * После успеха возвращает JWT-токен — клиент сразу логинит юзера.
     */
    @POST("auth/signup")
    suspend fun signup(
    ...
    /**
     * Запрос кода восстановления пароля (SMS). POST /auth/forgot-password
     */
    ...
    /**
     * Сброс пароля по коду из SMS. POST /auth/reset-password
     */
...
/**
 * FIX(2026-05-04): запрос самостоятельной регистрации.
 * Поля invite_code зарезервированы на V2 (одобрение куратором).
 */
@Serializable
data class SignUpRequest(
    ...
    // FIX(2026-05-23): мультироль при регистрации (до 3). Первая = основная (users.role),
    // остальные → user_role_assignments. Бэк auth_login.signup принимает roles[].
