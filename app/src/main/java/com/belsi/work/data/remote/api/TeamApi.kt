...
interface TeamApi {
    ...
    /**
     * Простой ВСЕЙ бригады (2.1.0) — POST /foreman/team/idle-all
     * Ставит все активные смены бригады в простой + критичный push куратору.
     */
    @POST("foreman/team/idle-all")
    suspend fun idleAllBrigade(
        @Body request: IdleAllRequest
    ): Response<IdleAllResponse>
}

/** Запрос «простой всей бригады» */
@kotlinx.serialization.Serializable
data class IdleAllRequest(
    val reason: String? = null,   // no_materials | waiting | vehicle_breakdown | other
    val comment: String? = null
)

/** Ответ «простой всей бригады» */
@kotlinx.serialization.Serializable
data class IdleAllResponse(
    val started: Int = 0,
    val skipped: Int = 0,
    val delivered: Int = 0
)
