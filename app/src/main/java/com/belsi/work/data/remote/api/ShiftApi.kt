...
interface ShiftApi {
    ...
    suspend fun uploadHourPhoto(
        ...
        // v3 привязка (Куда отнести фото?) — backend пишет site_object_window_photos.
        @Part("window_id") windowId: RequestBody? = null,
        @Part("stage") stage: RequestBody? = null,
        @Part("is_final") isFinal: RequestBody? = null
...
data class ShiftHistoryItem(
    val id: String,
    val start_at: String,
    // FIX(2026-05-23): сервер шлёт finish_at / duration_hours, а клиент читал end_at /
    // duration_minutes → поля всегда оставались null («Финиш —», «Чистое —»).
    // Маппим реальные имена ответа через @SerialName.
    @kotlinx.serialization.SerialName("finish_at") val end_at: String? = null,
    val status: String,
    @kotlinx.serialization.SerialName("duration_hours") val duration_hours: Double? = null,
