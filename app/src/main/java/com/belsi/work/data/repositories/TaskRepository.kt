...
class TaskRepositoryImpl @Inject constructor(
    ...
    // FIX(2026-05-05): offline-очередь для шатдаунов
    private val offlineQueue: com.belsi.work.data.offline.OfflineQueueRepository,
) : TaskRepository {
    ...
    override suspend fun updateTaskStatus(taskId: String, status: String): Result<Task> {
        return try {
            ...
            // FIX(2026-05-05): сеть упала → в offline-очередь, попробуется когда появится связь
            android.util.Log.w("TaskRepository", "Network down → queue updateTaskStatus", e)
            offlineQueue.enqueue(
                com.belsi.work.data.offline.PendingAction.UpdateTaskStatus(
                    taskId = taskId,
                    newStatus = status,
                )
            )
            Result.failure(
                com.belsi.work.data.offline.OfflineQueuedException(
                    "Изменение задачи: будет отправлено когда появится связь",
                    e,
                )
            )
        } catch (e: Exception) {
