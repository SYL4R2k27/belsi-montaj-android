...
interface TeamRepository {
    ...
    // Простой ВСЕЙ бригады (2.1.0)
    suspend fun idleAllBrigade(reason: String?, comment: String?): Result<com.belsi.work.data.remote.api.IdleAllResponse>
}
...
class TeamRepositoryImpl @Inject constructor(
    ...
    override suspend fun idleAllBrigade(
        reason: String?,
        comment: String?
    ): Result<com.belsi.work.data.remote.api.IdleAllResponse> {
        return safeApiCall("idleAllBrigade") {
            teamApi.idleAllBrigade(com.belsi.work.data.remote.api.IdleAllRequest(reason, comment))
        }
    }
}
