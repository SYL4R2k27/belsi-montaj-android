...
interface ToolTransferApi {
    ...
    // ─── Driver (FIX 2026-05-18: двух-стадийный flow от supplier до объекта) ───
    // POST /tools/transfers/{id}/pickup — driver подтверждает забор: dispatched → in_transit
    @POST("tools/transfers/{id}/pickup")
    suspend fun pickupTransfer(
        @Path("id") transferId: String,
        @Body body: Map<String, String> = emptyMap(),
    ): Response<Map<String, String>>

    // POST /tools/transfers/{id}/deliver — driver подтверждает доставку: in_transit → delivered
    @POST("tools/transfers/{id}/deliver")
    suspend fun deliverTransfer(
        @Path("id") transferId: String,
        @Body body: Map<String, String> = emptyMap(),
    ): Response<Map<String, String>>
