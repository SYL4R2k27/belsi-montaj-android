...
interface UserApi {
    ...
    /**
     * POST /user/me/current-object → sticky-binding пользователь<->объект.
     * site_object_id=null = открепиться.
     * FIX(2026-05-01): объект не теряется при minimize/kill app.
     */
    @POST("user/me/current-object")
    suspend fun setCurrentObject(
        @Body request: SetCurrentObjectRequest
    ): Response<User>
