...
import com.belsi.work.data.remote.api.SignUpRequest
...
interface AuthRepository {
    ...
    /**
     * FIX(2026-05-04): самостоятельная регистрация (V1).
     * После успеха возвращает AuthResult с токеном — клиент сразу логинит.
     */
    suspend fun signup(
        phone: String,
        password: String,
...
class AuthRepositoryImpl @Inject constructor(
    ...
    override suspend fun forgotPassword(phone: String): Result<Unit> {
        return try {
            val response = authApi.forgotPassword(com.belsi.work.data.remote.api.ForgotPasswordRequest(phone))
            if (response.isSuccessful) Result.success(Unit)
            else Result.failure(Exception(parseErrorMessage(response.code(), response.errorBody()?.string())))
        } catch (e: Exception) {
            Result.failure(Exception("Ошибка сети: ${e.message}", e))
        }
    }
    ...
    override suspend fun resetPassword(phone: String, code: String, newPassword: String): Result<Unit> {
        return try {
            val response = authApi.resetPassword(
                com.belsi.work.data.remote.api.ResetPasswordRequest(phone, code, newPassword)
            )
            if (response.isSuccessful) Result.success(Unit)
            else Result.failure(Exception(parseErrorMessage(response.code(), response.errorBody()?.string())))
        } catch (e: Exception) {
     ...
     * POST /auth/verify
     *
     * Response: { "token": "...", "phone": "+7XXXXXXXXXX" }
     */
    ...
    /**
     * FIX(2026-05-04): POST /auth/signup — V1 самостоятельная регистрация.
     * Без email/SMS подтверждения. Учётка сразу активна.
     * Сервер возвращает JWT-токен — сразу логинит юзера.
     */
    override suspend fun signup(
        ...
        return try {
            ...
            if (response.isSuccessful && response.body() != null) {
                ...
                prefsManager.saveUser(savedUser)

                // Регистрируем FCM-токен — чтобы welcome-push дошёл
                pushRepository.registerFcmToken()
            ...
            Result.failure(Exception("Ошибка регистрации: ${e.message}", e))
        }
    }
