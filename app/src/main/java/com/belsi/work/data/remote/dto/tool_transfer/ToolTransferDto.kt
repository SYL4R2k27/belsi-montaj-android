...
data class ToolTransferDto(
    ...
    // FIX(2026-05-18) BELSI 2.0.1 build7: tool kit (Тележка) metadata —
    // backend заполняет когда transfer создан через POST /supplier/tool-kits/{id}/dispatch.
    @SerialName("kit_batch_id") val kitBatchId: String? = null,
    @SerialName("kit_id") val kitId: String? = null,
    @SerialName("kit_name") val kitName: String? = null,
    @SerialName("kit_item_name") val kitItemName: String? = null,
    @SerialName("kit_item_section") val kitItemSection: String? = null,
    @SerialName("kit_team_index") val kitTeamIndex: Int? = null,
)
