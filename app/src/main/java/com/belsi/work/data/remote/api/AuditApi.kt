...
interface AuditApi {
    ...
    /**
     * FIX(2026-06-16): множество id пунктов, которые юзер КОГДА-ЛИБО принимал.
     * Используется Update Gate для показа плашки «по пунктам» (только
     * непринятые), а не «по версии приложения» (раньше плашка вылазила
     * при каждом обновлении, даже если пункты не менялись).
     */
    @GET("audit/consents/accepted")
    suspend fun getAcceptedConsentIds(): Response<AcceptedConsentsResponse>
}

@Serializable
data class AcceptedConsentsResponse(
