...
class MainActivity : ComponentActivity() {
    ...
    // FIX(2026-05-25): ФИКСИРУЕМ масштаб UI = натуральная (стабильная) плотность
    // устройства + fontScale 1.0, чтобы Samsung One UI «Screen zoom» (density) и
    // «Font size» (fontScale) НЕ раздували интерфейс.
    //
    // Проблема: на Galaxy Z Fold (раскрытом) при увеличенном One UI screenWidthDp
    // падал до ~600dp → двухпанельный master-detail схлопывался, KPI-текст переносился
    // ПО БУКВАМ («Акт/ивн/ых»), кнопки одобрения фото уезжали за экран. Адаптировать
    // под увеличенный масштаб = терять рабочее пространство. Решение — рендерить так,
    // как задумано (дизайн-плотность), игнорируя пользовательский зум/шрифт.
    //
    // DENSITY_DEVICE_STABLE = плотность устройства БЕЗ пользовательского Screen Zoom
    // (константа на устройство). На телефонах без зума == текущей → no-op; на Z Fold
    // с увеличенным One UI — возвращает дизайн-масштаб. attachBaseContext отрабатывает
    // при каждом создании Activity, а у MainActivity нет android:configChanges →
    // активити пересоздаётся при смене конфигурации (зум/fold/unfold) → блокировка
    // держится без onConfigurationChanged.
    override fun attachBaseContext(newBase: Context) {
        super.attachBaseContext(lockUiScale(newBase))
    }

    private fun lockUiScale(base: Context): Context {
        val stableDpi = DisplayMetrics.DENSITY_DEVICE_STABLE
        val cfg = Configuration(base.resources.configuration)
        val curDpi = cfg.densityDpi
        if (stableDpi > 0 && curDpi > 0 && curDpi != stableDpi) {
            // Пересчитываем dp-размеры экрана под новую плотность — иначе Configuration
            // станет несогласованным (densityDpi сменили, а screenWidthDp остался старым).
            cfg.screenWidthDp = cfg.screenWidthDp * curDpi / stableDpi
            cfg.screenHeightDp = cfg.screenHeightDp * curDpi / stableDpi
            cfg.smallestScreenWidthDp = cfg.smallestScreenWidthDp * curDpi / stableDpi
            cfg.densityDpi = stableDpi
        }
        cfg.fontScale = 1.0f
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val startDestination = try {
            if (prefsManager.getToken() != null) {
                // FIX(2026-05-22): гарантируем, что ТЕКУЩИЙ залогиненный профиль есть в
                // списке мульти-аккаунта. Иначе профили, вошедшие до появления фичи
                // (или не через LoginViewModel), не показывались в переключателе.
                prefsManager.getUser()?.let { u ->
                    val acctLogin = (u.phone ?: "")
                    if (acctLogin.isNotBlank()) {
                        prefsManager.upsertSavedAccount(
                            com.belsi.work.data.models.SavedAccount(
                                login = acctLogin,
                                name = u.fullName ?: u.firstName ?: u.phone ?: acctLogin,
                                phoneDisplay = u.phone,
                                role = u.role?.name?.lowercase(),
                            )
                        )
                    }
                }
        ...
        setContent {
            // FIX(2026-05-14): глобальный cap на fontScale = 1.1x.
            // На реальных Samsung One UI пользователи часто имеют system font scale
            // 1.15-1.3x (One UI по умолчанию ставит «средний» = ~1.15, а Display ▸
            // Font size часто выкрутут до 1.3). Это раздувает текст в кнопках,
            // ломает TabRow, NavigationRail и т.д. — мы видели на Z Fold 5.
            // Кап 1.1x — компромисс: даём baseline accessibility (текст чуть больше
            // если юзер просил), но не разрушаем layout-fidelity B2B-приложения.
            // Локальные PinnedFontScale (≤1.0) в NavRail/TabRow продолжают работать
            // поверх этого глобального cap'а.
            val systemDensity = LocalDensity.current
            val cappedDensity = remember(systemDensity.density, systemDensity.fontScale) {
                Density(
                    density = systemDensity.density,
                    fontScale = systemDensity.fontScale.coerceAtMost(1.1f),
                )
            }
            ...
            }  // CompositionLocalProvider(LocalDensity) — global fontScale cap 1.1x
        }
    }
    ...
    private fun handlePushIntent(intent: Intent?) {
        ...
        val target = when {
            ...
            // FIX(2026-05-18) BELSI 2.0.1: deep-link на kit_batch (Тележка).
            // data.kind = tool_kit_dispatched / tool_kit_delivered / tool_kit_accepted
            // data.kit_batch_id → ToolKitBatch screen
            extras.getString("kind")?.startsWith("tool_kit") == true -> {
                val batchId = extras.getString("kit_batch_id")
                if (!batchId.isNullOrBlank()) AppRoute.ToolKitBatch.createRoute(batchId)
                else AppRoute.ToolKitList.route
            }
